...
type ProtocolData = Record<string, string>;
...
function displayValue(value: string | undefined, unit = "") {
  const cleanValue = value?.trim();
  return cleanValue ? `${cleanValue}${unit ? ` ${unit}` : ""}` : "—";
}

function buildEnzymeSections(data: ProtocolData): PdfSection[] {
...
function buildElectrospinningSections(data: ProtocolData): PdfSection[] {
  return [
    {
      title: "Alapadatok",
      rows: [
...
function buildZetasizerSections(data: ProtocolData): PdfSection[] {
  return [
    {
      title: "Alapadatok",
      rows: [
...
function buildPlatereaderSections(data: ProtocolData): PdfSection[] {
...
function buildSections(protocol: ProtocolKind, data: ProtocolData) {
...
function createPage(): PageSurface {
...
async function canvasesToPdf(pages: PageSurface[]) {
  const { PDFDocument } = await import("./vendor/pdf-lib");
...
export async function createProtocolPdfBytes(
  ...
  const drawPageHeader = (surface: PageSurface) => {
  ...
  const drawRow = ({ label, value }: PdfRow) => {
  ...
  const drawNote = ({ paragraphs, tone = "purple" }: NonNullable<PdfSection["notes"]>[number]) => {
    const measureContext = page.context;
  ...
  const drawTable = ({
    ...
    const drawTableRow = (cells: string[], rowHeight: number, isHeader: boolean, rowIndex = 0) => {
      let x = MARGIN;
...
export async function downloadProtocolPdf(
  ...
  const filePart = (value: string | undefined, fallback: string) =>
